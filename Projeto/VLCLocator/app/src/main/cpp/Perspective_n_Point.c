...
*/
    ...
    matrix_free(pontos_real_teste, 2);
    //matrix_free(pontos_sensor_teste, 2);
    matrix_free_uint32_t(pontos_sensor_teste, 2);
    matrix_free(pontos_real, 6);
    //matrix_free(pontos_sensor, 6);
    matrix_free_uint32_t(pontos_sensor, 6);
...
void p3p_grunert_test(){
    ...
    matrix_free_uint32_t(pontos_sensor, 3);
    matrix_free(K, 3);
}
...
double ***p6p_dlt(uint32_t **pixel_coordinates, double **real_world_coordinates){
    ...
    */
    ...
    BB_inv = matrix_mult(p, B_inv, 3, 3, 3, 3);
    printf("BB_inv\n");
    ...
    qrqr = matrix_mult(qr[0], qr[1], 3, 3, 3, 3);
    printf("qrqr\n");
...
double *p4p_fischler1981_v2(double **K, double pixelSize, double **pontos_real, double **pontos_sensor){
    ...
    */
    ...
    //double or = or1 + or2 + 1;
    double orb = or1 + or2 + 1;

    double Xor;
    double Yor;

    //if((or / (lim[0][0] / lim[2][0])) < 0){
    if((orb / (lim[0][0] / lim[2][0])) < 0){
        Xor = 1;
    }else{
        Xor = -1;
    }
...
double **p3p_grunert(double **K, double **pontos_real, uint32_t **pontos_sensor){
    ...
    for(uint8_t i = 0; i < 4; i++){
        ...
        aux = b2 / (1 + pow(r[i][0], 2) - 2 * r[i][0] * cos_beta);
        if(aux < 0 || r[i][0] < 0){
        ...
        polinomio_s2 = (double*) malloc(3 * sizeof(double));
        polinomio_s2[0] = 1;
...
//z[0][0] = 0;z[1][0] = -0.3;z[2][0] = 0.9;
    ...
    matrix_free(pontos_real, 4);
    //matrix_free(pontos_sensor, 4);
    matrix_free_uint32_t(pontos_sensor, 4);
...
//z[0][0] = 0;z[1][0] = -0.3;z[2][0] = 0.9;
    ...
    p4p_grunert_gauss_newton(K, rX, sX);
    //matrix_print(K, 3, 3);
    //matrix_print(rX, 4, 4);
...
void p4p_grunert_test_v2_ideal(){
    ...
    double **Xo = matrix_create_random(3, 1, 0, 20);
    if(Xo[2][0] < 0){
        Xo[2][0] = -Xo[2][0];
    }
    ...
    p4p_grunert_gauss_newton(K, rX, sX_int);
